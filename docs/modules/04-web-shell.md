...
> 状态：⬜ 未开始　|　依赖：DM-02, DM-03　|　设计依据：skim_design.md §13
...
## 任务清单
- [ ] `app.py`：create_app + manifest 扫描注册 + 全局异常 + static/Jinja2
- [ ] `ws.py`：WSManager（connect 回放/disconnect/broadcast + buffer）
- [ ] `routes/dashboard.py`：全局首页（聚合各模块概览，无账号引导卡）
- [ ] `templates/base.html`：统一导航（采集/分析/制作/运营 tab）+ WS 状态指示器
- [ ] `templates/dashboard.html`
- [ ] `static/style.css` + `static/app.js`（WS 管理 + 进度更新 + 通知）
- [ ] 定义 manifest 契约文档（写入本文件 + 各模块 manifest.py 模板）
- [ ] 单测 `tests/core/test_routes.py`：TestClient 访问 / 、空库引导、SkimError→JSON
