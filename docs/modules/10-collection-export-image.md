...
## 实施记录
- `feat(export,image): DM-10 csv_exporter + image_downloader (30GB alarm)` — commit `08eb8e6`
  - csv_exporter.py: export_csv(ai/excel) + export_empty_db, direct SQLite read
  - image_downloader.py: async download_images (lazy httpx) + check_disk (30GB warn, configurable stop)
  - 17 tests: AI columns, top_comments pipe, Excel ZIP, empty DB, filtering, disk thresholds
  - loop_gate.sh x2 passed ✅
