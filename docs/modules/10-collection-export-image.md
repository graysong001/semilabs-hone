...
> 状态：⬜ 未开始　|　依赖：DM-02　|　设计依据：skim_design.md §14、§15
...
## 任务清单
- [ ] `csv_exporter.py`：AI 模式（top_comments 管道符）+ Excel 模式（ZIP 两 CSV）
- [ ] `csv_exporter.py`：空库处理 + 按 task_id/keyword 筛选
- [ ] `image_downloader.py`：异步下载（max_concurrency=4）+ 磁盘检查
- [ ] `image_downloader.py`：30GB warn + 可配硬停 + 剩余空间检查
- [ ] 单测 `tests/collection/test_csv_export.py`：AI 格式列头/top_comments、Excel ZIP 两 CSV、空库；`test_image_downloader`：磁盘阈值 mock
