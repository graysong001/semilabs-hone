...
> 状态：⬜ 未开始　|　依赖：DM-05, DM-06　|　设计依据：skim_design.md §10、§12
...
## 任务清单
- [ ] `slide_solver.py`：OpenCV 缺口检测 + DM-06 轨迹拖拽
- [ ] `ocr_solver.py`：ddddocr
- [ ] `manual_handler.py`：暂停 + WS 通知（经 IPC ws_events）
- [ ] `solver.py`：类型检测 + 分发
- [ ] `rhythm.py`：check_quiet_hours/check_daily_limit/note_delay/keyword_delay/should_pause_for_captcha
- [ ] `warmup.py`：random_browse
- [ ] 单测 `tests/collection/test_rhythm.py`：安静时段抛错、日限额、延迟区间、captcha 暂停阈值
