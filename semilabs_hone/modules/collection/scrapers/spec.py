...
class LoginSpec(BaseModel):
    ...
    [契约变更 2026-07-13 S10] 加 verify_url / identity_api / identity_map 三字段
    （均 optional，向后兼容已有 yaml）：
    - verify_url: 用注入 cookie 验证登录态有效的接口路径（GET，2xx=有效）
    - identity_api: 提取平台真实身份的接口路径（GET，返回 JSON）
